...
class FirebaseAuthManager {
  private val auth: FirebaseAuth? by lazy {
    try {
      if (FirebaseApp.getApps(com.example.StrawberrycandyApplication.instance).isEmpty()) {
        FirebaseApp.initializeApp(com.example.StrawberrycandyApplication.instance)
      }
      FirebaseAuth.getInstance()
    } catch (e: Exception) {
      Log.w("FirebaseAuthManager", "FirebaseAuth not available or uninitialized: ${e.message}", e)
      null
    }
  }
  ...
  suspend fun signIn(email: String, password: String): FirebaseAuthResult {
    ...
    val currentAuth = auth ?: return FirebaseAuthResult.Error("Firebase Auth service unavailable. Proceeding with local account session.")
    return try {
      val user = suspendCancellableCoroutine<FirebaseUser> { cont ->
        currentAuth.signInWithEmailAndPassword(cleanEmail, cleanPass)
  ...
  suspend fun signUp(email: String, password: String): FirebaseAuthResult {
    ...
    return try {
      val newUser = suspendCancellableCoroutine<FirebaseUser> { cont ->
        currentAuth.createUserWithEmailAndPassword(cleanEmail, cleanPass)
          .addOnSuccessListener { result ->
            val u = result.user
            if (u != null) {
              cont.resume(u)
  ...
  suspend fun sendPasswordResetEmail(email: String): Result<Unit> {
    val cleanEmail = email.trim().lowercase()
    val currentAuth = auth ?: return Result.failure(IllegalStateException("Firebase Auth not available"))
    return try {
      suspendCancellableCoroutine<Unit> { cont ->
        currentAuth.sendPasswordResetEmail(cleanEmail)
  ...
  suspend fun updateCurrentUserPassword(newPassword: String): Result<Unit> {
    val user = currentFirebaseUser ?: return Result.failure(IllegalStateException("No Firebase user logged in"))
    return try {
      suspendCancellableCoroutine<Unit> { cont ->
  ...
  /**
   * Signs in to Firebase Auth using a Google ID Token.
   */
  suspend fun signInWithGoogleIdToken(idToken: String): FirebaseAuthResult {
    ...
    val currentAuth = auth ?: return FirebaseAuthResult.Error("Firebase Auth not available")
    return try {
      ...
      Log.i("FirebaseAuthManager", "Successfully identified via Google Sign-In with Firebase Auth: ${user.email}")
